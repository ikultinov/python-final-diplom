    ...
    def post(self, request, *args, **kwargs):
        ...
        if {
            ...
            if request.data["password"] != request.data["password_rep"]:
                return JsonResponse(
                    {"Status": False, "Errors": "Password поля не совпадают!"}
                )
            ...
            try:
                ...
                if user_serializer.is_valid():
                    ...
                    return JsonResponse(
                        {"Status": True, "Message": "Ссылка отправлена на ваш email"}
                    )
                else:
                    return JsonResponse(
...
class ConfirmAccount(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if {"key"}.issubset(request.query_params):
            try:
                user_id = force_str(
                    urlsafe_base64_decode(request.query_params.get("key"))
                )
                ...
                user = None
                return JsonResponse(
                    {"Status": False, "Message": "Неправильная ссылка!"}, status=400
                )
            ...
            return JsonResponse(
                {
                    "Status": True,
                    "Message": "Ваш email подтвержден! Теперь можете авторизовываться для \
получения токена",
                }
            )
        else:
            return JsonResponse(
                {"Status": False, "Errors": "Неправильная ссылка!"}, status=400
            )
...
class ContactView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if len(count_contacts) == 5:
            return JsonResponse(
                {"Status": False, "Errors": "Не более 5 контактов пользователя!"},
                status=400,
            )
